 ...
 */
@Stateless
public class RelatorioEmpresasFacade extends AbstractFacade<RelatorioAreasEmpresas> {
    ...
    // Todas as Empresas relacionadas a uma determinada area de pesquisa selecionada
    public RelatorioAreasEmpresas[] findAllEmpresasPorArea(Areas area, String ordem) {        
        String sqlString = null;        
        if (null != ordem) switch (ordem) {
            case "empresa":
                sqlString = "SELECT ae.empresas_id,em.nome as empresa,"+
                "count(ae.empresas_id) as avaliadores, "+                         
                "(SELECT count(ae1.empresas_id) FROM gcompet.areas_empresas ae1 "+
                " WHERE ae1.areas_id = ae.areas_id AND ae.empresas_id = ae1.empresas_id "+
                " AND ae1.avaliacao = 1) as avaliacao " +                                                 
